...
def _msvc_toolchains_repo_impl(ctx):
    ...
    for winsdk_version in winsdk_versions:
        for msvc_version in msvc_versions:
            ...
            ctx.template(
                "toolchain_msvc_{msvc_version}_winsdk{winsdk_version}_{target}/args/BUILD.bazel".format(msvc_version = msvc_version, target = "x64", winsdk_version = winsdk_version),
                ctx.attr.src_args,
                substitutions = {
            ...
            ctx.template(
                "toolchain_msvc_{msvc_version}_winsdk{winsdk_version}_{target}/BUILD.bazel".format(msvc_version = msvc_version, target = "x64", winsdk_version = winsdk_version),
            ...
            ctx.template(
                "toolchain_msvc_{msvc_version}_winsdk{winsdk_version}_{target}/tools/BUILD.bazel".format(msvc_version = msvc_version, target = "x64", winsdk_version = winsdk_version),
                ctx.attr.src_tools,
                substitutions = {
                    "{msvc_repo}": "msvc_{}".format(msvc_version),
                },
...
"""
    ...
    for winsdk_version in winsdk_versions:
        for msvc_version in msvc_versions:
            root_build_file_content += """
toolchain(
    name = "msvc_{msvc_version}_winsdk{winsdk_version}_x64_toolchain",
    ...
    toolchain = "//toolchain_msvc_{msvc_version}_winsdk{winsdk_version}_x64:cc_toolchain",
    toolchain_type = "@bazel_tools//tools/cpp:toolchain_type",
)

    """.format(msvc_version = msvc_version, winsdk_version = winsdk_version)
